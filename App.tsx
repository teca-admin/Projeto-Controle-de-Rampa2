              ...
              {report ? (
                <div className="grid grid-cols-12 gap-4 md:gap-8 flex-1 overflow-hidden">
                  ...
                  <div className="hidden lg:flex col-span-3 flex-col gap-6 overflow-hidden">
                     <div className={`${isDarkMode ? 'bg-[#0f172a]/40 border-white/5' : 'bg-white border-slate-200'} border p-5 rounded-sm space-y-4 flex-none shadow-sm flex flex-col min-h-0`}>
                    ...
                    <div className="space-y-6 md:space-y-8">
                       ...
                       <div className={`${isDarkMode ? 'bg-[#0f172a]/30' : 'bg-white'} border ${themeClasses.border} p-5 md:p-6 shadow-xl rounded-sm transition-colors duration-300`}>
                          <div className="flex justify-between items-center mb-6">
                             ...
                             <button onClick={handleAddRental} className="bg-emerald-600 px-4 py-2 text-[10px] font-black uppercase italic rounded-sm shadow-lg active:scale-95 transition-all text-white flex items-center gap-2">
                          ...
                          <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1 custom-scrollbar">
                            {formRentals.length > 0 ? formRentals.map((loc, i) => (
                              <div key={i} className={`${themeClasses.bgInput} border ${themeClasses.border} p-4 rounded-sm flex flex-col relative group shadow-sm transition-all animate-in slide-in-from-right-2`}>
                                <button onClick={() => handleRemoveRental(i)} className="absolute -top-2 -right-2 bg-rose-600 p-1.5 rounded-full z-10 text-white shadow-lg opacity-0 group-hover:opacity-100 transition-all">
                                   <Trash2 size={12}/>
                                </button>
                                <div className="space-y-3">
                                   <div className="space-y-1">
