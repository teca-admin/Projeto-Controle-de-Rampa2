...
const App: React.FC = () => {
  ...
  const handleAddRental = () => setFormRentals([...formRentals, { equipamento: '', inicio: '', fim: '' }]);
  const handleRemoveRental = (index: number) => setFormRentals(formRentals.filter((_, i) => i !== index));
  const handleRentalChange = (index: number, field: string, value: string) => {
  ...
  const fetchData = useCallback(async (isSilent = false) => {
    try {
      ...
      let query = supabase.from('relatorios_consolidados').select('*').gte('data', startDate).lte('data', endDate);
      ...
      if (periodData) {
        ...
        periodData.forEach((curr: any) => {
          if (curr.locacoes && Array.isArray(curr.locacoes)) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'relatorios_consolidados' }, () => fetchData(true))
      .subscribe();
  ...
  const handleSaveReport = async () => {
    ...
    try {
      ...
      const locacoesParaSalvar = formRentals.map(r => ({
        equipamento: r.equipamento || "NÃO ESPECIFICADO",
      ...
      alert("Erro ao processar salvamento: " + err.message); 
    } finally { 
  ...
  const themeClasses = {
    ...
    navActive: isDarkMode ? 'bg-blue-600 text-white shadow-xl' : 'bg-blue-600 text-white shadow-lg',
    navInactive: isDarkMode ? 'text-slate-400 hover:text-white' : 'text-slate-400 hover:text-slate-600',
    shadow: isDarkMode ? 'shadow-2xl shadow-black/50' : 'shadow-md border border-slate-200/50'
  };
  ...
  return (
    <div className={`h-screen ${themeClasses.bgMain} ${themeClasses.textMain} flex flex-col font-sans selection:bg-blue-600/30 overflow-hidden transition-colors duration-300`}>
      <header className={`flex-none ${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white border-slate-200'} border-b px-4 md:px-6 py-4 flex justify-between items-center shadow-xl z-20`}>
        <div className="flex items-center gap-3 md:gap-4">
          <div className="bg-blue-600 p-1.5 rounded shadow-lg shadow-blue-500/40">
        ...
        <nav className={`hidden lg:flex ${isDarkMode ? 'bg-[#334155]' : 'bg-slate-100'} p-1 rounded-sm border ${themeClasses.border} gap-1`}>
        ...
        <div className="flex items-center gap-2 md:gap-3">
           ...
           <div className="flex items-center gap-1.5">
              <button onClick={toggleTheme} className={`hidden lg:flex p-2 md:p-2.5 ${isDarkMode ? 'bg-[#334155]' : 'bg-slate-100'} border ${themeClasses.border} rounded-sm hover:border-blue-500 transition-all text-slate-400 hover:text-blue-500 shadow-lg shadow-black/20`}>
                {isDarkMode ? <Sun size={16} /> : <Moon size={16} />}
              </button>
              <button onClick={() => fetchData()} className={`p-2 md:p-2.5 ${isDarkMode ? 'bg-[#334155]' : 'bg-slate-100'} border ${themeClasses.border} rounded-sm hover:border-blue-500 transition-all text-slate-400 hover:text-blue-500 shadow-lg shadow-black/20`}>
        ...
        <div className="h-full p-4 md:p-6 w-full max-w-[1900px] mx-auto flex flex-col">
          {loading ? (
                ...
                <div className="grid grid-cols-12 gap-4 md:gap-8 flex-1 overflow-hidden">
                  <div className="col-span-12 lg:col-span-9 flex flex-col gap-4 md:gap-6 overflow-hidden">
                     <div className="flex-none flex flex-col md:flex-row justify-between items-start md:items-end gap-4 md:gap-6">
                        ...
                        <div className={`${isDarkMode ? 'bg-[#1e293b]' : 'bg-white'} border ${themeClasses.border} p-3 md:p-4 rounded-sm shadow-2xl flex items-center gap-3 transition-colors duration-300`}>
                          <div className={`${isDarkMode ? 'bg-slate-700' : 'bg-slate-100'} p-2 rounded-sm transition-colors duration-300 shadow-inner`}><HardHat size={16} className="md:size-[18px] text-blue-500"/></div>
                          <div>
                     ...
                     <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-4">
                        {report.voos?.length ? report.voos.map((voo: any, idx: number) => (
                          <div key={idx} className={`${isDarkMode ? 'bg-[#1e293b] border-white/5' : 'bg-white border-slate-200'} border p-4 md:p-6 flex justify-between items-center group relative hover:border-blue-500/50 transition-all rounded-sm shadow-xl duration-300`}>
                            <div className="absolute left-0 top-0 bottom-0 w-1 bg-blue-600 opacity-0 group-hover:opacity-100 transition-all shadow-[0_0_15px_rgba(37,99,235,0.5)]"></div>
                            <div className="flex items-center gap-4 md:gap-6">
                              <div className={`${isDarkMode ? 'bg-[#334155]' : 'bg-slate-100'} p-4 border ${themeClasses.border} rounded-sm text-blue-500 group-hover:bg-blue-600 group-hover:text-white group-hover:shadow-[0_0_20px_rgba(37,99,235,0.3)] transition-all`}><Plane size={24}/></div>
                              <div>
                     ...
                     <div className="flex-none grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 pb-2">
                        <div className={`${isDarkMode ? 'bg-[#1e293b] border-amber-500/20' : 'bg-white border-amber-200'} border p-4 md:p-6 rounded-sm min-h-[100px] md:min-h-[140px] shadow-2xl transition-all hover:border-amber-500/40`}>
                          ...
                          <p className={`text-[10px] md:text-[11px] font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-600'} italic leading-snug`}>{report.descricao_pendencias || "Nenhuma pendência"}</p>
                        </div>
                        <div className={`${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white border-slate-200'} border p-4 md:p-6 rounded-sm min-h-[100px] md:min-h-[140px] shadow-2xl transition-all hover:border-white/20`}>
                          <div className="flex items-center gap-2 mb-3 md:mb-4">
                          ...
                          <p className={`text-[10px] md:text-[11px] font-bold ${isDarkMode ? 'text-slate-400' : 'text-slate-500'} italic leading-snug`}>{report.descricao_ocorrencias || "Não"}</p>
                        </div>
                     </div>
                  </div>
                  <div className="hidden lg:flex col-span-3 flex-col gap-6 overflow-hidden">
                     <div className={`${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white border-slate-200'} border p-5 rounded-sm space-y-4 flex-none shadow-2xl flex flex-col min-h-0`}>
                        <h4 className={`text-[10px] font-black uppercase italic tracking-[0.2em] ${isDarkMode ? 'text-slate-300' : 'text-slate-500'} flex items-center justify-between flex-none`}>Locações <Handshake size={14}/></h4>
                        <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar">
                          {report.locacoes && Array.isArray(report.locacoes) && report.locacoes.length > 0 ? report.locacoes.map((loc: any, lIdx: number) => (
                            <div key={lIdx} className={`${isDarkMode ? 'bg-[#334155] border-white/5' : 'bg-slate-50 border-slate-100'} p-4 border rounded-sm hover:border-blue-500/30 transition-all`}>
                             ...
                             <p className={`text-[9px] font-black ${isDarkMode ? 'text-slate-500' : 'text-slate-300'} uppercase italic py-2 text-center`}>Sem locações ativas</p>
                          )}
                        </div>
                     </div>
                     <div className={`${isDarkMode ? 'bg-[#1e293b] border-rose-600/30' : 'bg-rose-50 border-rose-200'} border p-5 rounded-sm space-y-4 flex-none shadow-2xl`}>
                        ...
                        ) : ( <p className={`text-[9px] font-black ${isDarkMode ? 'text-slate-500' : 'text-slate-300'} uppercase italic py-2 text-center`}>Toda frota operante</p> )}
                     </div>
                     <div className={`${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white border-slate-200'} border p-5 rounded-sm flex-1 flex flex-col min-h-0 shadow-2xl transition-colors duration-300`}>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                          {[ { l: 'Faltas', v: report.teve_falta, c: 'rose' }, { l: 'Atestados', v: report.teve_atestado, c: 'blue' }, { l: 'Compensação', v: report.teve_compensacao, c: 'slate' }, { l: 'Saída Antecipada', v: report.teve_saida_antecipada, c: 'emerald' } ].map(q => (
                            <div key={q.l} className={`flex justify-between items-center p-3 border-l-2 transition-all ${isDarkMode ? 'bg-[#0f172a]/30' : 'bg-slate-50'} ${q.v ? `border-${q.c}-500 shadow-[0_0_10px_rgba(0,0,0,0.2)]` : 'border-transparent'}`}>
            ...
            <div className="animate-in slide-in-from-right-5 duration-500 h-full flex flex-col gap-4 md:gap-6 overflow-hidden">
               <div className={`flex-none flex flex-col md:flex-row items-end gap-4 ${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white'} p-4 border ${themeClasses.border} rounded-sm shadow-2xl transition-colors duration-300`}>
                  ...
                  <button onClick={() => fetchData()} className="bg-blue-600 p-2.5 rounded-sm hover:bg-blue-500 transition-all active:scale-95 text-white shadow-xl shadow-blue-500/20"><RefreshCcw size={16} className={loading ? 'animate-spin' : ''}/></button>
               </div>
               ...
               <div className="flex-none grid grid-cols-2 md:grid-cols-6 gap-3 md:gap-4">
                  {[ 
                    ...
                    >
                      ...
                      <p className={`text-[7px] md:text-[8px] font-bold ${isDarkMode ? 'text-slate-500' : 'text-slate-400'} uppercase italic leading-none`}>{k.s}</p>
                    </div>
                  ))}
               </div>
               <div className="flex-1 grid grid-cols-12 gap-4 md:gap-6 overflow-hidden">
                  <div className={`col-span-12 lg:col-span-8 ${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white'} border ${themeClasses.border} p-6 md:p-8 rounded-sm shadow-2xl flex flex-col overflow-hidden transition-colors duration-300`}>
                  ...
                  <div className={`col-span-12 lg:col-span-4 ${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white'} border ${themeClasses.border} p-6 md:p-8 rounded-sm shadow-2xl flex flex-col overflow-hidden transition-colors duration-300`}>
                          ...
                          <div className="flex-1 overflow-y-auto pr-1 space-y-2 custom-scrollbar">
                            {fleetDetails.filter(e => e.status === 'OPERACIONAL').map(e => (
                              <div key={e.id} className={`${isDarkMode ? 'bg-[#0f172a]/30 border-r-2 border-emerald-500/0' : 'bg-slate-50 border-r-2 border-slate-100'} hover:border-emerald-500 p-2.5 transition-all flex justify-between items-center group cursor-default`}>
                                 <div><p className={`text-[10px] md:text-xs font-black italic tracking-tighter leading-none ${themeClasses.textHeader}`}>{e.prefixo}</p><p className={`text-[6px] md:text-[7px] font-bold ${isDarkMode ? 'text-slate-500' : 'text-slate-400'} uppercase italic mt-1 leading-none`}>{e.nome}</p></div>
            ...
            <div className="animate-in slide-in-from-right-5 duration-500 h-full flex flex-col gap-4 md:gap-6 overflow-hidden">
               <div className="flex-none flex flex-col md:flex-row justify-between items-end gap-4 md:gap-6">
                  ...
                  <div className={`${isDarkMode ? 'bg-[#1e293b]' : 'bg-white'} border ${themeClasses.border} flex items-center px-4 py-2 gap-3 w-full md:w-[350px] shadow-2xl focus-within:border-blue-500 transition-all`}><Search size={16} className={themeClasses.textMuted} /><input type="text" placeholder="CIA OU LÍDER..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className={`bg-transparent border-none focus:ring-0 text-[10px] font-black w-full italic ${isDarkMode ? 'text-white' : 'text-slate-900'}`} /></div>
               </div>
               <div className={`${themeClasses.bgCard} border ${themeClasses.border} shadow-2xl overflow-hidden flex flex-col transition-colors duration-300`}>
                  <div className={`flex-none grid grid-cols-6 ${isDarkMode ? 'bg-[#334155]' : 'bg-slate-100'} px-4 md:px-6 py-4 text-[8px] md:text-[9px] font-black ${themeClasses.textMuted} uppercase tracking-widest border-b ${themeClasses.border} italic`}>
                  ...
                  <div className="flex-1 overflow-y-auto divide-y divide-white/5 custom-scrollbar">
                     {allFlights.length > 0 ? allFlights.filter(f => !searchQuery || JSON.stringify(f).toLowerCase().includes(searchQuery.toLowerCase())).map((v, i) => (
                       <div key={i} onClick={() => { setSelectedDate(v.parentDate); setSelectedShift(v.parentShift === 'manhã' ? 'manha' : v.parentShift); setActiveTab('dashboard'); }} className={`grid grid-cols-6 px-4 md:px-6 py-4 md:py-5 items-center hover:bg-blue-600/10 transition-all cursor-pointer group`}>
                          ...
                          <div className="col-span-3 md:col-span-2 flex items-center gap-3 md:gap-4"><Plane size={16} className={`${isDarkMode ? 'text-slate-500' : 'text-slate-300'} group-hover:text-blue-500 transition-colors`} /><p className={`text-sm md:text-lg font-black italic tracking-tighter ${themeClasses.textHeader}`}>{v.companhia}</p></div>
                          ...
                          <div className="flex justify-end"><button className={`${isDarkMode ? 'bg-slate-700' : 'bg-slate-200'} px-3 md:px-4 py-1.5 md:py-2 text-[7px] md:text-[8px] font-black ${isDarkMode ? 'text-slate-300' : 'text-slate-600'} group-hover:bg-blue-600 group-hover:text-white transition-all uppercase italic shadow-lg shadow-black/30`}>Ver</button></div>
                       </div>
            ...
            <div className="animate-in slide-in-from-bottom-5 duration-500 h-full flex flex-col overflow-hidden pb-4">
               <div className="flex-1 overflow-y-auto pr-1 md:pr-2 custom-scrollbar space-y-6 md:space-y-8">
                    ...
                    <div className="space-y-2">
                       ...
                       <div className={`flex ${themeClasses.bgInput} p-1 border ${themeClasses.border} rounded-sm transition-colors duration-300 shadow-inner`}>
                          {(['manha', 'tarde', 'noite'] as const).map(t => (
                            <button key={t} onClick={() => setFormShift(t)} className={`flex-1 md:px-6 py-3 md:py-2.5 text-[10px] md:text-[9px] font-black uppercase italic rounded-sm transition-all ${formShift === t ? 'bg-blue-600 text-white shadow-xl shadow-blue-500/30' : isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>{t}</button>
                          ))}
                       </div>
                    </div>
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                    <div className="space-y-6 md:space-y-8">
                       <div className={`${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white'} border ${themeClasses.border} p-5 md:p-6 shadow-2xl rounded-sm transition-colors duration-300`}>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {[{k: 'falta', l: 'Falta'}, {k: 'atestado', l: 'Atestado'}, {k: 'compensacao', l: 'Compens.'}, {k: 'saida_antecipada', l: 'Saída Ant.'}].map(i => (
                              <button key={i.k} onClick={() => setFormHR({...formHR, [i.k as keyof typeof formHR]: !formHR[i.k as keyof typeof formHR]})} className={`p-5 md:p-4 border-2 md:border transition-all text-center rounded-sm ${formHR[i.k as keyof typeof formHR] ? 'bg-rose-500/20 border-rose-500/50 text-rose-500 shadow-lg shadow-rose-500/10' : `${themeClasses.bgInput} border-transparent opacity-40 shadow-inner`}`}>
                       ...
                       <div className={`${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white'} border ${themeClasses.border} p-5 md:p-6 shadow-2xl rounded-sm space-y-6 transition-colors duration-300`}>
                          <div className="space-y-2">
                             <label className="text-[10px] font-black text-amber-500 uppercase italic">2 - Pendências para o turno seguinte</label>
                             <textarea value={formPendencias} onChange={e => setFormPendencias(e.target.value)} rows={3} className={`${themeClasses.bgInput} border ${themeClasses.border} p-4 font-bold text-sm md:text-xs rounded-sm ${isDarkMode ? 'text-slate-300' : 'text-slate-900'} w-full italic outline-none focus:border-amber-500/30 transition-colors duration-300 shadow-inner`} placeholder="Descreva pendências aqui..."></textarea>
                          </div>
                          <div className="space-y-2">
                             <label className="text-[10px] font-black text-rose-500 uppercase italic">3 - Ocorrências / Avarias do Plantão</label>
                             <textarea value={formOcorrencias} onChange={e => setFormOcorrencias(e.target.value)} rows={3} className={`${themeClasses.bgInput} border ${themeClasses.border} p-4 font-bold text-sm md:text-xs rounded-sm ${isDarkMode ? 'text-slate-300' : 'text-slate-900'} w-full italic outline-none focus:border-rose-500/30 transition-colors duration-300 shadow-inner`} placeholder="Descreva ocorrências aqui..."></textarea>
                          </div>
                       </div>

                       <div className={`${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white'} border ${themeClasses.border} p-5 md:p-6 shadow-2xl rounded-sm transition-colors duration-300`}>
                          <div className="flex justify-between items-center mb-6">
                             <h4 className="text-[12px] md:text-[11px] font-black italic uppercase text-blue-500">4 - Controle de Locações</h4>
                             <button onClick={handleAddRental} className="bg-emerald-600 px-4 py-2 text-[10px] font-black uppercase italic rounded-sm shadow-xl shadow-emerald-600/20 active:scale-95 transition-all text-white flex items-center gap-2">
                                <Plus size={14}/> Nova Locação
                             </button>
                          </div>
                          
                          <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1 custom-scrollbar">
                            {formRentals.length > 0 ? formRentals.map((loc, i) => (
                              <div key={i} className={`${themeClasses.bgInput} border ${themeClasses.border} p-4 rounded-sm flex flex-col relative group shadow-inner transition-all animate-in slide-in-from-right-2`}>
                                <button onClick={() => handleRemoveRental(i)} className="absolute -top-2 -right-2 bg-rose-600 p-1.5 rounded-full z-10 text-white shadow-xl opacity-0 group-hover:opacity-100 transition-all">
                                   <Trash2 size={12}/>
                                </button>
                                <div className="space-y-3">
                                   <div className="space-y-1">
                                   ...
                                   <div className="grid grid-cols-2 gap-3">
                                      <div className="space-y-1">
                                         <label className={`text-[7px] font-black ${themeClasses.textMuted} uppercase`}>Início</label>
                                         <input type="time" value={loc.inicio} onChange={e => handleRentalChange(i, 'inicio', e.target.value)} className={`${isDarkMode ? 'bg-slate-900' : 'bg-white'} border-none p-2.5 font-black text-xs w-full text-blue-500 rounded-sm`} />
                                      </div>
                                      <div className="space-y-1">
                                         <label className={`text-[7px] font-black ${themeClasses.textMuted} uppercase`}>Término</label>
                                         <input type="time" value={loc.fim} onChange={e => handleRentalChange(i, 'fim', e.target.value)} className={`${isDarkMode ? 'bg-slate-900' : 'bg-white'} border-none p-2.5 font-black text-xs w-full text-blue-500 rounded-sm`} />
                                      </div>
                                   </div>
                                </div>
                              </div>
                              ...
                              <div className={`py-8 text-center border-2 border-dashed ${themeClasses.border} rounded-sm opacity-20`}>
                                <p className="text-[10px] font-black italic uppercase">Nenhuma locação adicionada</p>
                              </div>
                            )}
                          </div>
                       </div>
                    ...
                    <div className="space-y-6 md:space-y-8">
                       <div className={`${isDarkMode ? 'bg-[#1e293b] border-white/10' : 'bg-white'} border ${themeClasses.border} p-5 md:p-6 shadow-2xl rounded-sm flex flex-col transition-colors duration-300`}>
                         <div className="flex justify-between items-center mb-6">
                           <h4 className="text-[12px] md:text-[11px] font-black italic uppercase text-blue-500">5 - Log de Atendimentos</h4>
                           <button onClick={handleAddFlight} className="bg-blue-600 px-6 md:px-4 py-3 md:py-2 text-[11px] md:text-[9px] font-black uppercase italic rounded-sm shadow-xl shadow-blue-500/20 active:scale-95 transition-all text-white">+ Inserir CIA</button>
                         </div>
                         <div className="space-y-4 max-h-[450px] md:max-h-[400px] overflow-y-auto pr-1 md:pr-2 custom-scrollbar">
                           {formFlights.map((v, i) => (
                             <div key={i} className={`${themeClasses.bgInput} border ${themeClasses.border} p-5 md:p-4 rounded-sm flex flex-col relative group shadow-inner transition-colors duration-300`}>
                               <button onClick={() => handleRemoveFlight(i)} className="absolute -top-3 -right-3 md:-top-2 md:-right-2 bg-rose-600 p-2 md:p-1 rounded-full z-10 text-white shadow-xl shadow-black/40"><Trash2 size={16} className="md:size-3"/></button>
               ...
               <div className={`flex-none pt-4 md:pt-6 border-t ${themeClasses.border} flex flex-col md:flex-row justify-end items-stretch md:items-center gap-4 transition-colors duration-300`}>
                  <div className="flex gap-4">
                    ...
                    <button disabled={isSubmitting} onClick={handleSaveReport} className="flex-[2] md:w-[250px] bg-blue-600 hover:bg-blue-500 p-5 md:px-8 md:py-4 text-[13px] md:text-[11px] font-black text-white rounded-sm uppercase italic flex items-center justify-center gap-3 transition-all shadow-2xl shadow-blue-500/40 active:scale-95">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={`relative w-full max-w-2xl max-h-[80vh] ${themeClasses.bgCard} border ${themeClasses.border} shadow-2xl flex flex-col rounded-sm animate-in zoom-in-95 duration-200 overflow-hidden`}>
              ...
              {allRentalsInPeriod.length > 0 ? (
                <div className="space-y-4">
                  {allRentalsInPeriod.map((r, i) => (
                    <div key={i} className={`${isDarkMode ? 'bg-[#334155] border-white/10' : 'bg-slate-50 border-slate-200'} border p-4 rounded-sm flex justify-between items-center shadow-lg transition-all hover:bg-[#334155]/80`}>
                      <div>
            ...
            <div className="p-6 border-t border-white/10 flex justify-end">
              <button onClick={() => setIsRentalModalOpen(false)} className="bg-blue-600 px-6 py-2.5 text-[11px] font-black text-white uppercase italic rounded-sm shadow-xl shadow-blue-500/20">Fechar</button>
            </div>
          </div>
        </div>
      )}
